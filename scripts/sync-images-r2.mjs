 ...
 * Re-running resumes after checking existing R2 sizes, MIME and SHA-256 metadata.
...
async function retry(operation) {
  for (let attempt = 1; ; attempt++) {
    ...
    try {
      ...
      await new Promise(resolveWait => setTimeout(resolveWait, 750 * 2 ** (attempt - 1)));
    }
  }
}
...
async function matchesExisting(client, entry, sizes) {
  if (sizes.get(entry.key) !== entry.bytes) return false;
  ...
  return head?.ContentLength === entry.bytes &&
    head.ContentType === entry.contentType && head.Metadata?.sha256 === entry.sha256;
}
...
async function download(entry) {
  ...
  if (Number(response.headers.get('content-length') || 0) > entry.bytes) {
    await response.body?.cancel();
    throw new Error('Image source exceeds its verified byte size.');
  }
  const chunks = [];
  let bytes = 0;
  for await (const chunk of response.body) {
    bytes += chunk.byteLength;
    if (bytes > entry.bytes || bytes > maxBytes) throw new Error('Image source exceeds its verified byte size.');
    chunks.push(chunk);
  }
  return validateBytes(Buffer.concat(chunks), entry);
}
        ...
        try {
          ...
          await retry(async () => {
            ...
            await client.send(new PutObjectCommand({
              Bucket: bucket, Key: entry.key, Body: body, ContentLength: entry.bytes,
